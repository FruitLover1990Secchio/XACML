...
class PolicyBuilder{
  ...
  _build_constructor(total_private_functions){
    ...
    for (let k = 1; k<= total_private_functions; k++){
      declarations += `euint8 public immutable grade_threshold_${k};\n    `;
      constructor += `grade_threshold_${k} = FHE.asEuint8(27);\n      FHE.allowThis(grade_threshold_${k});
      `;
    }
...
contract SmartPolicy_${this.value_checks}_${total_private_functions} is SepoliaConfig {
    enum RuleEffect {
        PERMIT,
        DENY,
        NOTAPPLICABLE,
